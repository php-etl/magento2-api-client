...
class V1OrderOrderIdInvoicePostBodyNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return $type === 'Kiboko\\Magento\\V2_4\\Model\\V1OrderOrderIdInvoicePostBody';
    }
    public function supportsNormalization($data, $format = null): bool
    {
        return is_object($data) && get_class($data) === 'Kiboko\\Magento\\V2_4\\Model\\V1OrderOrderIdInvoicePostBody';
    }
    /**
     * @return mixed
     */
    public function denormalize($data, $class, $format = null, array $context = array())
    {
        ...
        $object = new \Kiboko\Magento\V2_4\Model\V1OrderOrderIdInvoicePostBody();
        ...
        if (\array_key_exists('arguments', $data)) {
            $object->setArguments($this->denormalizer->denormalize($data['arguments'], 'Kiboko\\Magento\\V2_4\\Model\\SalesDataInvoiceCreationArgumentsInterface', 'json', $context));
        }
        if (\array_key_exists('capture', $data)) {
            $object->setCapture($data['capture']);
        }
        if (\array_key_exists('comment', $data)) {
            $object->setComment($this->denormalizer->denormalize($data['comment'], 'Kiboko\\Magento\\V2_4\\Model\\SalesDataInvoiceCommentCreationInterface', 'json', $context));
        }
        if (\array_key_exists('items', $data)) {
            $values = array();
            foreach ($data['items'] as $value) {
                $values[] = $this->denormalizer->denormalize($value, 'Kiboko\\Magento\\V2_4\\Model\\SalesDataInvoiceItemCreationInterface', 'json', $context);
            }
